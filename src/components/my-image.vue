<template>
  <div class="my-image">
    ...
    <!--
      src动态绑定后, 本地图片加载失败,
      如果是网络路径, 是可以加载成功的,
      项目基于vue-cil,基于webpack, 打包所有的资源
      但是只会绑定静态资源不会绑定动态资源
      解决方案: 主动去导入默认图片数据, 在src上绑定数据即可
    -->
    ...
    <el-dialog :visible.sync="dialogVisible" width="750px">
      <el-tabs v-model="activeName" type="card">
        <el-tab-pane label="素材库" name="image">
          ...
          <!-- 列表 -->
          <div class="img-list">
            <!-- @click="selectedImageUrl=item.url"记录当前图片点击的地址 -->
            <div
              class="img-item"
...
.img-btn {
  width: 160px;
  height: 160px;
  border: 1px dashed rgb(214, 24, 24);
  img {
    width: 100%;
    height: 100%;
    display: block;
...
.img-list {
  ...
  .img-item {
    width: 160px;
    height: 120px;
    border: 1px dashed rgb(219, 40, 40);
    display: inline-block;
    margin-right: 15px;
    margin-bottom: 10px;
    position: relative;
